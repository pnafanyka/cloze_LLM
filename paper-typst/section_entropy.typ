...
*Model entropy.* For GPT-4o-mini, the API returns an unnormalized probability distribution over multi-token continuations: the sum of `probability_converted` per context ranges from 0.05 to 1.99 (median 0.39), with 5 of 144 contexts exceeding 1.0.#footnote[Sums exceeding 1.0 arise because the model predictions include both partial prefixes and their completions as separate entries. For example, for the context targeting _банке_, both the prefix "бан" ($p = 0.996$) and the full continuation "банке" ($p = 0.996$) appear as distinct rows --- their probabilities are per-path in the token tree, not per-leaf, so they are not mutually exclusive. The same pattern occurs in all 5 affected contexts (e.g., "покрыв"/"покрывало").] We compute entropy over the raw (non-renormalized) probabilities:
...
Across all 144 contexts, the mean human entropy is $H_"human" = 2.18$ nats and the mean model entropy is $H_"model" = 0.84$ nats. The model's partial distribution is substantially more concentrated than the human response distribution.
...
#table(
  ...
  [Q1 (low)], [1.06], [0.73], [0.425], [0.743],
  [Q2], [1.97], [0.99], [0.284], [0.486],
  [Q3], [2.49], [0.86], [0.168], [0.292],
  [Q4 (high)], [3.20], [0.78], [0.102], [0.220],
)
]

The pattern is monotonic: as human uncertainty increases from Q1 to Q4, lemma overlap\@10 drops from 42.5% to 10.2%, and weighted overlap\@10 drops from 74.3% to 22.0%. A similar trend holds across all values of $K$: even at $K = 100$, Q4 contexts reach only 12.5% overlap compared to 43.5% for Q1.

Notably, model entropy does not increase in parallel with human entropy. Q4 contexts (highest human uncertainty) actually have the _lowest_ mean model entropy (0.78 nats), while Q2 contexts show the highest (0.99 nats). The model remains confident regardless of whether human respondents agree or disagree.
